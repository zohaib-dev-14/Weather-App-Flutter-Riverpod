...
class WeeklyForecastSearchview extends ConsumerWidget {
  final String? cityName;
  const WeeklyForecastSearchview({super.key, required this.cityName});

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final weeklyForecast = ref.watch(getWeeklyWeatherProviderByPoints(cityName ?? ""));
    return weeklyForecast.when(data: (weeklyWeather) {
      return GridView.builder(

          gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2,
          childAspectRatio: 0.8,
          crossAxisSpacing: 18, mainAxisSpacing: 15),
    ...
    }, error: (error, stackTrace) => Center(child: Text(stackTrace.toString()),), loading: () {
      return const Center(
...
class WeeklyWeatherTile extends ConsumerWidget {
  const WeeklyWeatherTile({super.key, required this.day, required this.date, required this.temp, required this.icon});
  final String day;
  final String date;
  final double temp;
  final int icon;
...
const SizedBox(height: 7),
            ...
            Text(formattedDate,
             ...
             SuperscriptText(text: temp.toString(), color: AppColors.white, superScript: "°C", superScriptColor: AppColors.grey),
             const SizedBox(height: 20),
              SizedBox(
                   height: 51,
                  child: Image.asset(getWeatherIcon2(icon))),
